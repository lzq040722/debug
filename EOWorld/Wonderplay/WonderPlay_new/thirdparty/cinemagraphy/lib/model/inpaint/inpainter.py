...
class Inpainter():
    def __init__(self, device='cuda', ckpt_dir='ckpts'):
        self.device = device
        print("Loading depth model...")
        depth_feat_model = Inpaint_Depth_Net()
        depth_feat_weight = torch.load(
            os.path.join(ckpt_dir, 'depth-model.pth'), map_location=torch.device(device)
        )
        ...
        rgb_feat_weight = torch.load(
            os.path.join(ckpt_dir, 'color-model.pth'), map_location=torch.device(device)
        )
