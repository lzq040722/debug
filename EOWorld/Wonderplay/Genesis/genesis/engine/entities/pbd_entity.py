...
class PBDParticleEntity(ParticleEntity):
    ...
    def process_input(self, in_backward=False):
        ...
        # 使用Taichi kernel进行批量更新
        if self._tgt["pos"] is not None:
            self._kernel_set_particles_pos(self._tgt["pos"])

        if self._tgt["vel"] is not None:
            self._kernel_set_particles_vel(self._tgt["vel"])

        if self._tgt["act"] is not None:
            assert self._tgt["act"] in [gs.ACTIVE, gs.INACTIVE]
            self._kernel_set_particles_active(self._tgt["act"])
    ...
    @ti.kernel
    def _kernel_set_particles_pos(self, pos: ti.types.ndarray()):
        for i in range(self.n_particles):
            i_v = i + self._particle_start
            for j in ti.static(range(3)):
                self.solver.particles[i_v].pos[j] = pos[i, j]

    @ti.kernel
    def _kernel_set_particles_active(self, act: ti.i32):
        for i in range(self.n_particles):
            i_v = i + self._particle_start
            self.solver.particles_ng[i_v].active = act

    @ti.kernel
    def _kernel_set_particles_vel(self, vel: ti.types.ndarray()):
        for i in range(self.n_particles):
            i_v = i + self._particle_start
            # 创建临时速度向量
            ...
            # 如果速度大于10，进行裁剪
            if v_norm > 10.0:
                v = v * (10.0 / v_norm)  # 保持方向，但限制大小为10
            
            # 设置裁剪后的速度
            for j in ti.static(range(3)):
                self.solver.particles[i_v].vel[j] = v[j]
