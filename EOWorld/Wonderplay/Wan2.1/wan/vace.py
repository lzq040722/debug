...
class WanVace(WanT2V):
    ...
    def generate(self,
        ...
        Args:
            ...
            init_video (`torch.Tensor`, *optional*):
                Init video for SDEdit, with shape [C, F, H, W] in [-1, 1].
            sdedit_strength (`float`, *optional*, defaults to 1.0):
                SDEdit strength in [0, 1]. 1.0 keeps the original random init.
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            # sample videos
            latents = noise
            if init_video is not None and sdedit_strength < 1.0:
                if not 0.0 <= sdedit_strength <= 1.0:
                    raise ValueError(
                        f"sdedit_strength must be in [0, 1], got {sdedit_strength}"
                    )
                init_latent = self.vae.encode([init_video.to(self.device)])[0]
                if init_latent.shape[0] != target_shape[0]:
                    raise ValueError(
                        f"init_video latent channel mismatch: {init_latent.shape} vs {target_shape}"
                    )
                if init_latent.shape[2:] != tuple(target_shape[2:]):
                    raise ValueError(
                        f"init_video latent spatial mismatch: {init_latent.shape} vs {target_shape}"
                    )
                if init_latent.shape[1] > target_shape[1]:
                    init_latent = init_latent[:, -target_shape[1]:]

                init_samples = noise[0].clone()
                init_samples[:, -init_latent.shape[1]:] = init_latent
                start_index = min(
                    int((1.0 - sdedit_strength) * len(timesteps)),
                    len(timesteps) - 1)
                sample_scheduler.set_begin_index(start_index)
                start_timestep = timesteps[start_index:start_index + 1]
                latents = [
                    sample_scheduler.add_noise(
                        init_samples.unsqueeze(0),
                        noise[0].unsqueeze(0),
                        start_timestep).squeeze(0)
                ]
                timesteps = timesteps[start_index:]
                logging.info(
                    f"SDEdit init_video enabled: strength={sdedit_strength}, "
                    f"start_index={start_index}, start_timestep={timesteps[0].item()}"
                )
