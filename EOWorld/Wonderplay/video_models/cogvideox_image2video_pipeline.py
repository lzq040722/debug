...
class CogVideoXImageToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __call__(
        ...
        step_ratio: Optional[float] = -1.0,
        timesteps: Optional[List[int]] = None,
        guidance_scale: float = 6,
        use_dynamic_cfg: bool = False,
        num_videos_per_prompt: int = 1,
        eta: float = 0.0,
        sdedit_strength: float = 1,
        mask_strength: float = -1,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        latents: Optional[torch.FloatTensor] = None,
        bg_latents: Optional[torch.FloatTensor] = None,
        ...
        if mask_strength > 0:
            ...
            bg_dropin_timestep = None

        with self.progress_bar(total=num_inference_steps) as progress_bar:
            # for DPM-solver++
            old_pred_original_sample = None
            for i, t in enumerate(timesteps):
                if mask_strength > 0 and sdedit_strength < 1 and t == bg_dropin_timestep:
                    latents = torch.where(masks, latents, bg_latents)
                if self.interrupt:
                    continue
                ...
                if not isinstance(self.scheduler, CogVideoXDPMScheduler):
                    latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, step_ratio=step_ratio, return_dict=False)[0]
